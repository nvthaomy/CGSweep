...
def CreateForceField(Sys, Cut, UseLocalDensity, CoordMin, CoordMax, LDKnots, RunSpline, 
                        NSplineKnots, NumberGaussians, GaussMethod, SplineKnots, BondFunc, BondSplineMethod, HarmonicParam):
    ...
    if RunSpline:
        PSpline = sim.potential.PairSpline(Sys, Filter = Filter, Cut = Cut,
                                           ...
                                           NonbondEneSlopeInit = "50.kTperA", BondEneSlope = "50.kTperA")
        if FitSpline:
            Max = 100.
            decay = 1.
...
def CreateSystem(Name, BoxL, NumberMolecules, NumberMonomers, Cut, UseLocalDensity, CoordMin, CoordMax,
                    LDKnots, RunSpline, NSplineKnots, NumberGaussians, BondFunc, BondSplineMethod, HarmonicParam):
    ...
    FFList, FFGaussians, NumberGaussians, opt = CreateForceField(Sys, Cut, UseLocalDensity, CoordMin, CoordMax, LDKnots, 
                                                            ...
                                                            NumberGaussians, GaussMethod, SplineKnots, BondFunc, BondSplineMethod, HarmonicParam) 
...
HarmonicParam = []
if BondSplineMethod == 1:
    BondFuncs = ['harmonic', 'spline']
else:
    BondFuncs = ['spline']
#Create and run optimization for different functional form of bonded potential
for BondFunc in BondFuncs:
    SysList = []
    OptList = []
    ...
    if RunOptimization:
        ...
        if BondFunc == 'harmonic':
            ...
            print (Dict)
            OptimizerPrefix =  ("{}_HarmonicBond".format(SrelName))
            RunSrelOptimization(Optimizer, OptimizerPrefix, UseWPenalty, StageCoefs, MaxIter=None, SteepestIter=0)
            #get harmonic bond params
            Dict = SysFFList[0][0][0].ParamDict()
            print ('Harmonic bond parameters:')
            print (Dict)
            HarmonicParam.append(Dict['FConst'])
